...
namespace InboxWatcher
{
    ...
    {
        private static readonly MailModelContainer Context = new MailModelContainer();
        ...
        public static void LogEmailReceived(IMessageSummary summary, IClientConfiguration config)
        {
            ...
            var email = new Email()
            {
                BodyText = "", EnvelopeID = summary.Envelope.MessageId, InQueue = true,
                MarkedAsRead = false, Minutes = (int)(DateTime.Now.ToLocalTime() - summary.Date.LocalDateTime).TotalMinutes,
                Sender = summary.Envelope.From.ToString(), Subject = summary.Envelope.Subject, TimeReceived = summary.Date.LocalDateTime, ImapMailBoxConfigurationId = config.Id
            };
        ...
        {
            var selectedEmails = Context.Emails.Where(x => config.Id == x.ImapMailBoxConfigurationId && x.EnvelopeID.Equals(email.Envelope.MessageId));
